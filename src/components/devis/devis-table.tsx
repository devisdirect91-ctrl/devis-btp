...
import { clientDisplayName, STATUS_LABELS, STATUS_STYLES } from "@/lib/client-utils"
          ...
          return (
            <div key={d.id} className="bg-white rounded-2xl border border-slate-100 p-4">
              <div className="flex items-start justify-between gap-3 mb-2">
                <Link href={`/devis/${d.id}`} className="text-xs font-mono text-slate-500 bg-slate-100 px-2 py-1 rounded">
                  {d.numero}
                </Link>
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${STATUS_STYLES[displayStatus]}`}>
                  {STATUS_LABELS[displayStatus]}
                </span>
              </div>
              <Link href={`/devis/${d.id}`} className="block mb-3">
                <p className="font-medium text-slate-900 text-sm leading-tight">{d.titre}</p>
                <p className="text-xs text-slate-400 mt-0.5">{clientDisplayName(d.client)}</p>
              </Link>
              ...
              <div className="flex items-center gap-2 pt-3 border-t border-slate-50">
                ...
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-medium text-blue-600 bg-blue-50 active:bg-blue-100 rounded-xl transition-colors"
                >
