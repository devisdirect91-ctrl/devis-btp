...
export function CatalogueView({ items }: CatalogueViewProps) {
  ...
  const handleSeed = () => {
    ...
    startSeedTransition(async () => {
      ...
      if (!res.ok) setSeedError(data.error ?? `Erreur ${res.status}`)
      else router.refresh()
    })
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="hidden md:block p-8 max-w-7xl mx-auto">
          ...
          <ItemSlideOver isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} onSaved={handleSaved} item={editingItem} />
        </div>
      </div>
    )
  }
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap flex-shrink-0 transition-all ${
                activeCategory === "ALL" ? "bg-violet-500 text-white" : "bg-white border text-gray-600"
              }`}
            >
              Tous ({items.length})
            </button>
            {usedCategories.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap flex-shrink-0 transition-all ${
                    activeCategory === cat ? "bg-violet-500 text-white" : "bg-white border text-gray-600"
                  }`}
                >
