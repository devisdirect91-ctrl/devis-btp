...
import { FilePlus, Plus, Search, Receipt, CheckCircle, Clock, AlertTriangle } from "lucide-react"
...
export default async function FacturesPage({ searchParams }: PageProps) {
  ...
  const joursRetard = (dateEcheance: Date | string) => {
    const diff = Math.floor(
      (now.getTime() - new Date(dateEcheance).getTime()) / (1000 * 60 * 60 * 24)
    )
            ...
            grouped.map((group) => (
              <div key={group.key} className="mb-6">
                ...
                <div className="space-y-2">
                  {group.items.map((f) => {
                    const displayStatus = computeFactureStatus(
                      f.status,
                    ...
                    const montantRestant = f.totalTTC - f.montantPaye
                    const jours = isRetard ? joursRetard(f.dateEcheance) : 0
                    const clientName = clientDisplayName(f.client)
                    ...
                    const s = statusStyle[displayStatus] ?? statusStyle.EN_ATTENTE
                    ...
                    return (
                      <div key={f.id} className="bg-white rounded-xl border overflow-hidden">
                          ...
                          <div className="flex justify-between items-center mt-3 pt-2 border-t border-gray-100">
                            <div className="flex items-center gap-1.5 text-xs text-gray-400">
                              ...
                              {isRetard ? (
                                ...
                                <span>Payée le {fmtDate(f.datePaiement)}</span>
                              ) : (
                                <span>Éch. {fmtDate(f.dateEcheance)}</span>
                              )}
                            </div>
                            ...
                            {isPartiel ? (
                              <p className="text-sm">
                                <span className="font-bold text-blue-600">
                                  {fmtMontant(f.montantPaye)}
                                </span>
                                <span className="text-gray-400 font-normal text-xs">
                                  {" "}/ {fmtMontant(f.totalTTC)}
                                </span>
                              </p>
                            ) : (
                        ...
                        {/* Bouton relance — hors du Link principal */}
